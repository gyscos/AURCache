...
async fn backfill_multi_dep_package() {
    ...
    let libaegis = packages::Entity::find()
        .filter(packages::Column::Name.eq("libaegis"))
        .one(&db)
        .await
        .unwrap()
    ...
    let simsimd = packages::Entity::find()
        .filter(packages::Column::Name.eq("simsimd"))
        .one(&db)
        .await
        .unwrap()
    ...
    let turso = packages::Entity::find()
        .filter(packages::Column::Name.eq("turso"))
        .one(&db)
        .await
        .unwrap()
...
async fn backfill_resolves_provider_dependencies() {
    ...
    let provider = packages::Entity::find()
        .filter(packages::Column::Name.eq("provider-pkg"))
        .one(&db)
        .await
        .unwrap()
