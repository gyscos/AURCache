...
fn base_name(pkg: &packages::Model) -> String {
    // For AUR packages the pkgbase is already stored in source_data JSON.
    // No need to call the AUR service again.
    serde_json::from_str::<serde_json::Value>(&pkg.source_data)
        .ok()
...
async fn mark_duplicate_pending_builds_failed(db: &impl ConnectionTrait) -> Result<(), DbErr> {
    ...
    let mut pending = builds::Entity::find()
        .filter(
            builds::Column::Status.is_in([Some(ACTIVE_BUILD_STATUS), Some(ENQUEUED_BUILD_STATUS)]),
        )
        .all(db)
        .await?;

    // Sort to determine winner per (pkg_id, platform):
    // active builds first, then most recent start_time, then highest id.
    pending.sort_by(|a, b| {
        let priority = |s: Option<i32>| if s == Some(ACTIVE_BUILD_STATUS) { 0 } else { 1 };
        priority(a.status)
...
async fn create_pending_build_unique_index(manager: &SchemaManager<'_>) -> Result<(), DbErr> {
    ...
    let index_sql = format!(
        "CREATE UNIQUE INDEX IF NOT EXISTS idx_builds_pending_pkg_platform ON {schema}builds (pkg_id, platform) WHERE status IN ({ACTIVE_BUILD_STATUS}, {ENQUEUED_BUILD_STATUS});"
    );
