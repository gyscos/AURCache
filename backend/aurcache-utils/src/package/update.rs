    ...
    let update_context = match source_data {
        packages::SourceData::Aur { .. } => {
            let (upstream_version, deps) = client
                .deps_of_with_version(pkg_model.name.as_str())
                .await
            ...
            UpdateContext {
                upstream_version,
                split_packages: split_packages_json(&pkg_model.name, &deps.pkgnames)?,
...
async fn sync_dependency_graph(
    ...
    let dep_constraints_by_pkgbase =
        resolve_dependency_constraints_by_pkgbase(client, db, &pkg_model.name, &dep_constraints)
            .await?;
...
async fn dependencies_ready_for_platform(
    ...
    for (dep_pkgbase, constraint) in dep_constraints_by_pkgbase {
        ...
        if dependency_satisfies_constraint(db, dep_pkg.id, platform, constraint.as_ref()).await? {
            continue;
        }
