...
async fn scenario_a_no_aur_deps() {
    ...
    let pkg = Packages::find()
        .filter(packages::Column::Name.eq("no-deps-pkg"))
        .one(&env.db)
        .await
        .unwrap()
...
async fn scenario_b_one_aur_dep() {
    ...
    assert_eq!(
        parent_builds.len(),
        1,
        "parent should have a WAITING_FOR_DEPS build"
    );
    assert_eq!(
...
async fn scenario_c_cascade_after_dep_build() {
    ...
    let parent_build_count = builds::Entity::find()
        ...
        .unwrap();
    assert_eq!(
        parent_build_count, 1,
        "parent should have a WAITING_FOR_DEPS build"
    );
    let parent_build = builds::Entity::find()
        .filter(builds::Column::PkgId.eq(parent.id))
        .one(&env.db)
        .await
        .unwrap()
...
async fn scenario_d_make_dep_only() {
    ...
    let dep_pkg = Packages::find()
        .filter(packages::Column::Name.eq("make-env-pkg"))
        .one(&env.db)
        .await
        .unwrap()
    ...
    let tool = Packages::find()
        .filter(packages::Column::Name.eq("build-tool"))
        .one(&env.db)
        .await
        .unwrap()
...
async fn scenario_e_shared_dep_no_duplicate() {
    ...
    let child_pkgs = Packages::find()
        .filter(packages::Column::Name.eq("child"))
        .all(&env.db)
        .await
        .unwrap();
    ...
    let parent1 = Packages::find()
        .filter(packages::Column::Name.eq("parent-1"))
        .one(&env.db)
        .await
        .unwrap()
        .unwrap();
    let parent2 = Packages::find()
        .filter(packages::Column::Name.eq("parent-2"))
        .one(&env.db)
        .await
        .unwrap()
        .unwrap();
...
async fn scenario_f_system_deps_only() {
    ...
    let pkg = Packages::find()
        .filter(packages::Column::Name.eq("my-pkg"))
        .one(&env.db)
        .await
        .unwrap()
        .expect("package should exist");
...
async fn scenario_g_split_package_constraints_are_merged_per_pkgbase() {
    ...
    let shared = Packages::find()
        .filter(packages::Column::Name.eq("shared-base"))
        .one(&env.db)
        .await
        .unwrap()
...
async fn scenario_h_provider_dependency_resolves_to_aur_package() {
    ...
    let provider = Packages::find()
        .filter(packages::Column::Name.eq("virtual-provider"))
        .one(&env.db)
        .await
        .unwrap()
...
async fn scenario_j_local_queued_provider_prevents_aur_dependency_addition() {
    ...
    let local_provider = Packages::find()
        .filter(packages::Column::Name.eq("local-provider"))
        .one(&env.db)
        .await
        .unwrap()
    ...
    let aur_provider_count = Packages::find()
        .filter(packages::Column::Name.eq("aur-provider"))
        .count(&env.db)
        .await
        .unwrap();
    assert_eq!(
...
async fn scenario_k_self_resolved_dependency_is_ignored() {
    ...
    let pkg = Packages::find()
        .filter(packages::Column::Name.eq("self-base"))
        .one(&env.db)
        .await
        .unwrap()
        .expect("package should exist");
...
async fn scenario_h_queue_missing_buildable_packages_after_migration() {
    ...
    assert_eq!(
        mid_builds.len(),
        1,
        "non-leaf dependency should get a WAITING_FOR_DEPS build"
    );
    assert_eq!(
        mid_builds[0].status,
        Some(BuildStates::WAITING_FOR_DEPS),
...
async fn scenario_i_queue_non_leaf_packages_when_dependencies_are_already_built() {
    ...
    assert_eq!(
        root_builds.len(),
        1,
        "root should have a WAITING_FOR_DEPS build while mid is not yet built"
    );
    assert_eq!(
...
async fn scenario_j_queue_only_platforms_with_satisfied_dependencies() {
    ...
    let root_x86_builds = builds::Entity::find()
        .filter(builds::Column::PkgId.eq(root.id))
        .filter(builds::Column::Platform.eq(Platform::X86_64))
        .count(&env.db)
        .await
        .unwrap();
    ...
    let root_aarch64_builds = builds::Entity::find()
        .filter(builds::Column::PkgId.eq(root.id))
        .filter(builds::Column::Platform.eq(Platform::Aarch64))
        .all(&env.db)
        .await
        .unwrap();
    assert_eq!(
        root_aarch64_builds.len(),
        1,
        "aarch64 should have a WAITING_FOR_DEPS build until its dependency is built"
    );
    assert_eq!(
